...
@objc @IBDesignable public class CCMRadarView: UIView{
        ...
        if let sublayers = layer.sublayers {
            for (index,sublayer) in (layer.sublayers as [CALayer]!).enumerated() {
                if let sublayer = sublayer as? CAShapeLayer {
                    ...
                    animation.keyTimes = [0, NSNumber(value: Double(beginTime/animation.duration)), NSNumber(value: beginTime/animation.duration), NSNumber(value: (beginTime + Double(animation.duration)/(Double(numberOfWaves) - 2.5))/animation.duration)]
    ...
    private func initialSetup(){
        ...
        for _ in 0..<numberOfWaves {
            ...
            let circle = UIBezierPath(ovalIn: sublayer.bounds.insetBy(dx: waveWidth, dy: waveWidth))//UIBezierPath(ovalInRect: sublayer.bounds.insetBy(dx: waveWidth, dy: waveWidth))
