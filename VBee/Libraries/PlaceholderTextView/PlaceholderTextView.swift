...
@IBDesignable class PlaceholderTextView: UITextView {
    ...
    func listenForTextChangedNotifications() {
        NotificationCenter.default.addObserver(self, selector: #selector(self.textChangedForPlaceholderTextView), name:NSNotification.Name.UITextViewTextDidChange , object: self)
        NotificationCenter.default.addObserver(self, selector: #selector(self.textChangedForPlaceholderTextView), name:NSNotification.Name.UITextViewTextDidBeginEditing , object: self)
    }
    ...
    override func draw(_ rect: CGRect) {
        ...
        if self.text.characters.count == 0 && self.placeholder != nil {
            let baseRect = placeholderBoundsContainedIn(containerBounds: self.bounds)
